...
const getPlaylistFromPlatform = async (req, res) => {
  ...
  const playlists = await spotifyService.getPlaylistFromPlatform(
...
const getItemFromPlatform = async (req, res) => {
  ...
  for (const playlist of playlists) {
    const track = await spotifyService.getItemFromPlatform(
      playlist.platformId,
      platformToken.accessToken
    );
...
const createPlaylistToPlatform = async (req, res) => {
  ...
  for (let i = 0; i < playlists.length; i++) {
    // const newPlaylist = await spotifyService.createPlaylistToPlatform(
    //   playlists[i],
    //   platformId,
    //   accessToken
    // );
    ...
    const trackIds = await spotifyService.getTrackIdFromPlatform(
      tracks[i],
      accessToken
    );
    ...
    //   await spotifyService.insertTrackToPlatform(
    //     newPlaylist.id,
    //     trackId,
    //     accessToken
    //   );
